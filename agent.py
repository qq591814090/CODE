...
class IdaStarSearchAgent(SearchAgent):
    ...
    def __init__(self):
        ...
        self.goal = None # we have no idea where to go at first
        self.length = {(0,0):0}
        self.m = {(0,0): manhattan_heuristic(0,0)}
        self.bound = 16
    ...
    def dfs_action(self, observations):
            ...
            for m, (dr,dc) in enumerate(MAZE_MOVES):
                ...
                if not observations[2 + m]: # can we go that way?
                    if (r2,c2) not in self.visited:
                        ...
                        self.length[(r2,c2)] = self.length[(r,c)] + 1
                        self.m[(r2,c2)] = self.length[(r2,c2)] + manhattan_heuristic(r2,c2)
        ...
        if self.goal == None:
            for i in range(len(self.queue)):
                ...
                if(minbound >= self.m[(temp[0],temp[1])]):
                    minbound = self.m[(temp[0],temp[1])]
                if self.m[(temp[0],temp[1])] <= self.bound:
                    self.goal = temp
                    break;
                else:
